const { decorateIcons, loadScript, loadCSS } = await import(
...
function parseConfig(block) {
  ...
  if (block.classList.contains('hero')) {
    ...
    const button = block.querySelector(
      'div > div:nth-child(2) > p:last-child > a',
    );
...
function getPosterImage(posterElement) {
  ...
  const supportedSources = [...sources].filter((source) => {
    ...
    return (
      ...
      && (window.matchMedia(media).matches || !media)
    );
  });
